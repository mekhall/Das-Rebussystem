...
För att använda detta system behövs en webbserver med php-stöd samt php-sqlite och php-json. Enklaste sättet att sätta upp en sådan är att använda docker compose:

    docker compose up

Efter detta finns servern på `http://localhost:8080`.

Inställningar för nginx-servern som används finns i `default.conf`.

Om du vill bygga din egen docker image, se README.docker.md.
...
Operationer är: <->, i, om, ! och byten
Byten skrivs som en kommaseparerad lista: A->B, +A, -B
Kreativa associationer kan ha en förklaring inom parantes:

    \op ! (förklaring)

Det finns ett script för att kolla att rebusarna är korrekta:

    ./check.py ht2011/S8.txt

Använd detta! Trasiga rebusar på presentationen kan bli tråkigt...
